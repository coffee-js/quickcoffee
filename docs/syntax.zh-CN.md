...
内建 `qtest --json` 每个文件输出一行稳定 JSON，供 CI 与宿主系统使用；`qtest --tap` 输出确定性的 TAP 13 记录；`qcoffee --fingerprint FILE` 在不执行脚本时输出已验证字节码的稳定 16 位十六进制键；`qbench --json` 输出带语义护栏的编译、验证、执行计时记录；`--stats` 的执行统计仍写入标准错误。
