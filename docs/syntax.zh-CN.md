...
`CompileLimits` 独立限制单份原始 UTF-8 source（默认 1,000,000 bytes）、单个 Program/Module 递归可达的 instructions（1,000,000）、静态图唯一 canonical modules（1,024）和累计原始图 source（16,000,000 bytes）。source 在 `.litcoffee` 提取与 cache-key 复制前检查，失败编译不查询或污染 Runtime cache；`Context::run_program` 会按所属 Runtime 策略复核外部或宿主构造的 Program。模块执行与图指纹共享同一图预算，且执行会在任何模块脚本运行前完成全图加载、编译、解析和验证。`qcoffee` 通过 `--max-source-bytes`、`--max-bytecode-instructions`、`--max-module-graph-modules` 与 `--max-module-graph-source-bytes` 暴露同一策略。`ResourceLimit` 是 non-exhaustive，嵌入方匹配时必须保留通配分支。

显式选择的 contextual native 会收到 `NativeCallContext` 与参数，可协作检查取消、读取 `ResourceLimits`、按确定性宿主工作量扣减 fuel、记录 logical managed object/byte 遥测、downcast 一个归 Context 所有的 `HostState`，并从 `HostCapabilities` 获取显式 allowlist 中的 typed handle。`CapabilityKey<T>` 把每个脚本不可见 slot 分类为 clock、random、logging、file 或 network，但不提供任何系统实现；缺失或类型不匹配返回 `None`。模块子 Context 继承同一 handles，独立 Context 默认隔离，host bindings 不进入 globals、序列化、Runtime cache 或 retained-memory census。既有 `add_native` 回调保留简单路径；同步回调在调用线程内执行，不能被强制中断，宿主 panic 不会被捕获或事务回滚。Runtime/Context 明确 non-`Send`/non-`Sync`，只有 `CancellationToken` 可跨线程；0.x embedding API 遵循 RFC 0154 的 patch/minor 迁移契约。
...
`ResourceLimits::with_max_transient_managed_objects` 与 `with_max_transient_managed_bytes` 另行设置默认关闭的单轮累计分配上限。RFC 0146 既有 VM/标准库记账点在写入完整 delta 后立即检查；contextual native 经协作 `record_managed_allocation` 参与，静态模块图共享同一聚合账本。越界不可捕获，会保留失败点统计并恢复脚本拥有的 Context 可变状态；下一次顶层执行重新开始预算。它仍不同于 RSS、allocator capacity、逐时刻 live 峰值和 retained graph。
...
模块由嵌入宿主显式加载：`ModuleLoader` 只返回规范名称和源码，`Context::run_module` 在私有顶层环境执行并只返回声明的 `ModuleExports`；同名依赖在一次运行中复用，循环依赖明确报错，且整张图共享 fuel 与取消边界。`Engine::fingerprint_module_graph` 在同一 loader 边界内加载并验证完整静态图但不执行，将入口、原始 source、Program 指纹、imports/exports 与规范依赖边编码为顺序无关的版本化缓存键；缺失和 cycle 不产生部分键。`Engine::prepare_module_package` 与 `Runtime::prepare_module_package` 可将完整预检图变为不可变、内存内的快照；`Context::run_module_package` 绝不调用 loader，并在每次运行建立新的私有模块 globals 与 exports。源码变化时由宿主显式重建 package。宿主可显式构造 `RestrictedFileModuleLoader`，只读取一个规范根目录内的 UTF-8 `.coffee` 或 `.litcoffee` 文件；依赖必须使用 `./` 或 `../`，省略扩展名时补 `.coffee`，绝对/bare 名、平台专用分隔符、词法越界及符号链接逃逸均拒绝。CLI 用 `qcoffee --module-root ROOT ENTRY` 显式执行同一图，或用 `qcoffee --fingerprint --module-root ROOT ENTRY` 只检查图指纹；普通 CLI 运行不会隐式读取模块。
