...
显式选择的 contextual native 会收到 `NativeCallContext` 与参数，可协作检查取消、读取 `ResourceLimits`、按确定性宿主工作量扣减 fuel、记录 logical managed object/byte 遥测，并 downcast 一个归 Context 所有的 `HostState`；模块子 Context 继承同一 handle。host state 对脚本不可见，也不进入 globals、序列化、Runtime cache 或 retained-memory census。既有 `add_native` 回调保留简单路径；同步回调不能被强制中断，宿主 panic 不会被捕获，且此 API 不授予 ambient capability。
