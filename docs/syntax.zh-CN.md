...
标准库是普通函数：`print`、`len`、`type`、`error`、`range`、`str`、`integer`、`number`、`abs`、`sum`、`min`、`max`、`keys`、`values`、`join`、`split` 与 `assert`。聚合只收同质的有限 Number 或 Integer 数组；`sum([])` 为 Number `0`，`min([])` 与 `max([])` 报错。
