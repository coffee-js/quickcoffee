...
字符串索引与严格切片按 Unicode 标量边界：`'a☕中'[1]` 为 `'☕'`，`'a☕中'[1..2]` 为 `'☕中'`；负索引从末项计数（`items[-1]`），越界仍报错；字符串 `for` 亦支持按 Unicode 标量下标以正有限整数 `by` 步进，例如 `for character, index in 'a☕中x' by 2 then index` 得 `[0, 2]`。
