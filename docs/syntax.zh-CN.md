...
| OO/模块 | 无原型工厂类 `class Name(args) -> expression`；仅嵌入 `Engine::compile_module` 的命名 `import { x as y } from 'name'`、`export x = value` 与 `export { local as public }`；宿主可显式构造根目录受限的 `RestrictedFileModuleLoader` | 继承、`this`、`new`、CLI 文件模块、隐式文件/网络加载 |
