...
`qbench --compare-qjs PATH --compare-iterations 1 --repeat 11 --json` emits a separate `quickcoffee.qcompare.v1` same-machine CLI comparison. The caller supplies PATH; records include startup, parsing, compilation, and execution, so they must not be compared directly with in-process `qbench.v1` fields.
