...
其内府皆常函，如 print、len、type、error、range、str、trim、contains、starts_with、ends_with、parse_json、encode_json、integer、number、decimal、decimal_div、round_decimal、abs、sum、min、max、keys、values、join、split、assert；RFC 0139 之字询严而无 locale，trim 依固定 Unicode White_Space 表；error(code, message, data, cause) 作密封 Error，catch 得之，而资源之误不可捕。Decimal 以 m 为缀，除不尽者须明定数位与舍法。
