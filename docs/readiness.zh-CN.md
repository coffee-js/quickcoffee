...
性能方面，当前 VM 尚未达到与 QuickJS 相近的量级。issue #95 的稳定环境槽位、当前作用域名称提示与借用式调度把同机 11 样本的预编译标量热循环从约 `36.5×` 降至 **17.70×**，函数热循环降至 **24.89×**；七条集合负载现已全部进入 `20×` 内，固定键映射读取为 **12.92×**。首要的标量 `10×` 目标仍未达到，因此下一阶段应继续处理静态 local/capture slot、调用与成员读取，而不能宣称已经达到 QuickJS 量级。这些结果仍不足以外推到异常、较大数据集或真实宿主调用。
