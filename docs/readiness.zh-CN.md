...
本文件是 2026-08-26 在 RFC 0000–0144 上的工程评估，不改变既有语言语义，也不是跨语言兼容性声明。动态优先级、验收状态和最新实测由 GitHub issues 维护。
...
它尚不适合独立承担多文件服务、长期作业编排或需要丰富通用库的应用：嵌入 API 已有宿主控制的静态模块核心，CLI 也只能通过显式 `--module-root ROOT ENTRY` 启用受限 `.qc` 文件图，但模块包、图指纹与预编译 manifest 尚未交付；异步/并发、正则、日期时间、网络能力和宿主对象能力模型也仍缺失。脚本已有无隐式 I/O、精确 Integer/Decimal 映射和 Context 可配置资源守卫的 JSON 编解码，RFC 0139–0140 提供固定 Unicode 查询与稳定标量排序，RFC 0144 再增加资源有界、不可变的 String/Array 连接。一般 String/Array/Map 单值边界已经覆盖字面量、宿主值及输出增长，但模块、总内存、对象数与生命周期预算仍由 #76 跟踪。这些不是遗漏的 JavaScript 兼容项；其中 I/O、时钟、随机和网络应保持为宿主显式注入的 capability，而非语言隐式全局。
