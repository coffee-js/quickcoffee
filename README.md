...
当前实现遵循 [RFCs/0000-project-scope.md](RFCs/0000-project-scope.md) 至 [RFCs/0130-program-source-maps.md](RFCs/0130-program-source-maps.md)。
...
嵌入宿主可用 `compile_named`、`compile_program_named` 或 `Context::eval_named` 把虚拟文档名原样附到结构化错误 label；匿名 API 保持不变。经 `Program` 编译的顶层、嵌套函数、默认参数和模块运行期错误也保留 source map；宿主手工构造的裸 `Chunk` 不虚构来源。`qcoffee --json FILE` 的错误在位置已有来源时额外输出可选 `source` 字段。
