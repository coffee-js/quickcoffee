...
当前实现遵循 [RFCs/0000-project-scope.md](RFCs/0000-project-scope.md) 至 [RFCs/0129-ast-lowering-validation-spans.md](RFCs/0129-ast-lowering-validation-spans.md)。
...
嵌入宿主可用 `compile_named`、`compile_program_named` 或 `Context::eval_named` 把虚拟文档名原样附到结构化错误 label；匿名 API 保持不变。`qcoffee --json FILE` 的错误在位置已有来源时额外输出可选 `source` 字段。
