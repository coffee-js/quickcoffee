...
`--stats` 适用于实际执行模式（`-e SOURCE`、FILE、`-` 或 `--interactive`）。交互模式仅为成功执行或产生运行时错误的非空输入行输出一条统计；解析、验证错误不执行字节码，也不输出新的统计（`last_execution()` 按 RFC 0066 保留上一条记录）。`:help`、`:quit` 和空行同样不执行字节码，因而不输出统计。与 `--check`、`--dump-bytecode` 同时使用是参数错误，退出码为 `2`；这两种模式不执行字节码，因而没有本次执行统计。
