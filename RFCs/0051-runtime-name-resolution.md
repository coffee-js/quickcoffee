...
issue #104 对上述快帧再作一层保守判定：只有调用入口时每个静态局部槽位都已由必填参数初始化，才不分配空的子 `Environment`，而直接把捕获环境作为非局部名称的查找起点。包含条件局部变量或其他未初始化槽位的函数仍保留子环境及原有 unset fallback；因此环境消除不把局部写入泄漏到父作用域。共享 `Program` 在不同 Context 布局中复用时，捕获环境的正命中提示仍验证完整名称，失配即回退动态查找。
