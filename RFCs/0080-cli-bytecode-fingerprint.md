...
`--fingerprint` 与 `--check`、`--dump-bytecode`、`--interactive`、`--stats` 互斥；冲突或缺少文件参数为用法错误，退出码为 2。读取、解析或验证错误沿用普通 CLI 的非零退出与结构化错误文本。相同源码必须产生相同指纹，不同有效字节码内容应产生不同指纹（碰撞概率遵循 64 位指纹的通常边界）。
