...
Builtin 函数值驻留在每线程只读父环境中，每个 `Context` 持有独立可写子环境；首次解析某 builtin 时只把该绑定提升到当前 Context 的稳定 slot，避免热调用重复穿越父环境。`set_global` 与脚本赋值仍可在当前 Context shadow 同名 builtin，但不能修改其他 Context 或共享父环境。该布局使 Context 构造成本不随标准库条目数量线性复制，同时保留既有宿主隔离语义。
