...
RFC 0151 在同一 `ModuleLoader` 权限边界上增加 `Engine::fingerprint_module_graph`：它加载并验证完整静态图但不执行模块，以版本化 canonical encoding 为宿主缓存提供依赖敏感指纹。缺失模块与 cycle 沿用本 RFC 的结构化错误，不生成部分图键。
