...
预置函数是普通名称，不是对象原型方法：`print(value...)`、`len(value)`、`type(value)`、`range(start, end)`、`str(value)`、`trim(text)`、`contains(text, needle)`、`starts_with(text, prefix)`、`ends_with(text, suffix)`、`abs(number)`、`sum(array)`、`min(array)`、`max(array)`、`keys(map)`、`values(map)`、`join(array, separator)`、`split(string, separator)`、`assert(bool, message?)`。数值聚合只接受有限数；空数组的 `sum` 为 `0`，`min`/`max` 要求非空。RFC 0139 的字符串查询严格、大小写敏感且不读取 locale；`trim` 使用 RFC 固定的 Unicode White_Space 表。不存在 `console`、`Object.prototype`、`Array.prototype` 或 String prototype。
