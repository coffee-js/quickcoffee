...
普通 QuickCoffee 源码使用 `.coffee`，以便 GitHub Linguist 和编辑器按 CoffeeScript 高亮；`.litcoffee` 是文学源码。旧的 `.qc` 不再是源码扩展名，工具不得发现、推断或加载它。仓库的 Linguist 属性将两种规范扩展名都声明为 CoffeeScript。
...
`.litcoffee` 使用 CoffeeScript 的 Markdown 文学形式：未缩进文本是正文；在文件开头或与正文至少相隔一个空行、以四个空格或一个 tab 开始的连续行是代码。预处理只移除一层文学缩进，同一文件的代码块必须统一使用四空格或 tab。正文被替换为空行，因此源码行号保持不变。
