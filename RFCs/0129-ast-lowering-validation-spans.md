...
5. 本切片不向公开 `Chunk`、`Instruction` 或常量编码加入 span，不改变 disassembly 与 `Program::fingerprint()`。RFC 0130 已通过私有 `Program` sidecar 完成成功表达式/statement 的运行期归因，RFC 0131 将该 sidecar 用于 mapped verification 与调用链上下文；裸 bytecode 刻意仍无来源。
