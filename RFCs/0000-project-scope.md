...
当前已实现的后续语义与工具 RFC 延伸至 RFC 0125；其中 RFC 0077–0118 维持既有 JSON、测试、指纹、文档、资源、性能与发布契约，RFC 0119 定义宿主控制的静态模块核心，RFC 0120 定义 QuickJS CLI 对比基准，RFC 0121 定义稳健的基准离散度报告，RFC 0122 定义 VM 热点计数，RFC 0123 定义托管值与词法环境分配剖析，RFC 0124 分离跨运行时启动、编译与预编译热执行阶段，RFC 0125 建立 CoffeeScript 2016 官方语言参考的实现、改写与拒绝矩阵；它们均不改变脚本语言值模型的原型无关约束。
