...
比较输出使用独立 schema `quickcoffee.qcompare.v1`，每个负载包含 `name`、`iterations`、`repeat`、`expected`、`quickcoffee_cli_ns` 与 `quickjs_cli_ns`。`--compare-iterations` 控制每个采样内的 CLI 次数，默认一；`--repeat` 仍控制样本数并取上侧中位数。建议构建全部 CLI 后以 `--compare-iterations 1 --repeat 11 --json` 运行。
