...
`qbench --json` 每条记录新增稳定的 `schema` 与 `version` 字段：`schema` 固定为 `quickcoffee.qbench.v1`，`version` 为当前 crate 版本。已有 `name`、`iterations`、`expected`、`compile_ns`、`verify_ns`、`execute_ns` 字段保持不变；三个 `*_ns` 字段仍表示本进程内指定迭代次数的纳秒总耗时，而非单次平均值。
