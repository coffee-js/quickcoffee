...
issue #100 的编译器解析叶函数槽位沿用同一口径。槽位化的参数与局部读写仍按原字节码计入 `name_loads` / `name_stores`；为保持跨版本可比性，逻辑函数帧仍计一次 `environment_allocations`，即使该隔离帧的局部值不再写入名称环境。issue #104 允许入口槽位全部初始化的快帧完全省去物理 `Environment` 分配，但仍保留这一次逻辑事件。因此该字段是稳定的 VM 分配模型，不等同于底层分配器调用次数。
