...
本轮加入 `string-escapes` 负载，语义护栏要求转义后的 `ABC` 长度与比较结果合计为 `4`。三轮 release 运行的原始样本（编译 / 执行，单位为 ms）为 `64.647 / 65.290 / 65.683` 与 `44.006 / 32.444 / 33.232`；中位数及吞吐如下：

| 工作负载 | 编译中位数 | 编译吞吐 | 执行中位数 | 执行吞吐 |
|---|---:|---:|---:|---:|
| string-escapes (20,000 次) | 65.290 ms | 306,326 programs/s | 33.232 ms | 601,830 programs/s |
