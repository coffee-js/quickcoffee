...
## RFC 0072 增量：Unicode 字符串索引与切片

本轮加入 `string-indexing` 负载，语义护栏要求对 `'a☕中'` 重复取标量索引与切片后累计为 `300`。三轮 release 运行的原始样本（单位为 ms）为编译 `74.234 / 72.707 / 73.655`、验证 `3.072 / 3.123 / 3.200`、执行 `904.437 / 856.151 / 877.041`；中位数及吞吐如下：

| 工作负载 | 编译中位数 | 编译吞吐 | 验证中位数 | 验证吞吐 | 执行中位数 | 执行吞吐 |
|---|---:|---:|---:|---:|---:|---:|
| string-indexing (10,000 次) | 73.655 ms | 135,768 programs/s | 3.123 ms | 3,201,665 programs/s | 877.041 ms | 11,402 programs/s |
...
字符串索引、切片和 `len` 均按 Unicode 标量计数，不暴露 UTF-8 字节偏移；三轮完整基准均通过语义护栏。
